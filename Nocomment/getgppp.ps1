function getgppp {
    ...
    try {
        ...
        Write-Verbose '[getgppp] Searching local host for any cached GPP files'
        ...
        if ($SearchForest) {
            Write-Verbose '[getgppp] Searching for all reachable trusts'
        ...
        ForEach ($Domain in $Domains) {
            Write-Verbose "[getgppp] Searching \\$Domain\SYSVOL\*\Policies. This could take a while."
        ...
        if ( -not $XMLFiles ) { throw '[getgppp] No preference files found.' }

        Write-Verbose "[getgppp] Found $($XMLFiles | Measure-Object | Select-Object -ExpandProperty Count) files that could contain passwords."
